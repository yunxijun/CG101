...
void CBezierPatch::Tessellation(CMesh Mesh)//细分曲面函数
{
	...
	for(int i = 0;i < 4; i++)
	{
		...
		vTangent = (u3 * P3[0][0] + u2 * P3[1][0] + u1 * P3[2][0] + u0 * P3[3][0]) * dv3
		...
		quadrN[i] = CrossProduct(CVector3(uTangent), CVector3(vTangent)).Normalize();//规范化顶点法向量
	}
...
void CBezierPatch::DrawFacet(CDC* pDC, CZBuffer* pZBuffer)
{
	...
	SlideProjector.SetEye(1000);
	for (int nPoint = 0; nPoint < 4; nPoint++)
	{
		ScreenPoint[nPoint] = projection.PerspectiveProjection3(quadrP[nPoint]);//透视投影
		quadrT[nPoint] = SlideProjector.TexturePerspectiveProjection(quadrP[nPoint], pTexture);
	}
	pZBuffer->SetPoint(ScreenPoint[0], ScreenPoint[2], ScreenPoint[3], quadrN[0], quadrN[2], quadrN[3], quadrT[0], quadrT[2], quadrT[3]);
	pZBuffer->PhongShader(pDC, ViewPoint, pLight, pMaterial, pTexture);
	pZBuffer->SetPoint(ScreenPoint[0], ScreenPoint[1], ScreenPoint[2], quadrN[0], quadrN[1], quadrN[2], quadrT[0], quadrT[1], quadrT[2]);
	pZBuffer->PhongShader(pDC, ViewPoint, pLight, pMaterial, pTexture);
}
...
void CBezierPatch::DrawControlGrid(CDC* pDC)//绘制控制网格
{
	...
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 4; j++)
			P2[i][j] = projection.PerspectiveProjection3(CtrPt[i][j]);//透视投影
	CPen NewPen, *pOldPen;
