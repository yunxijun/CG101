...
void CBezierPatch::Tessellation(CMesh Mesh)//细分曲面函数
{
	...
	for(int i = 0;i < 4; i++)
	{
		...
		quadrN[i] = CrossProduct(CVector3(uTangent), CVector3(vTangent)).Normalize();//归一化顶点法向量
	}
}
...
void CBezierPatch::DrawFacet(CDC* pDC, CZBuffer* pZBuffer)
{
	...
	pZBuffer->SetPoint(ScreenPoint[0], ScreenPoint[2], ScreenPoint[3], quadrN[0], quadrN[2], quadrN[3]);//绘制上三角形
	pZBuffer->PhongShader(pDC, ViewPoint, pLight, pMaterial);
	pZBuffer->SetPoint(ScreenPoint[0], ScreenPoint[1], ScreenPoint[2], quadrN[0], quadrN[1], quadrN[2]);//绘制下三角形
	pZBuffer->PhongShader(pDC, ViewPoint, pLight, pMaterial);
}
