...
CTestView::CTestView() noexcept
{
	...
	revoDown.patch.SetTexture(&texture);
	revoUp.patch.SetTexture(&texture);
}
...
void CTestView::InitializeLightingScene(void)//初始化光照环境
{
	...
	pMaterial->SetEmission(CRGB(0.2, 0.2, 0.2));//自身辐射的颜色
...
void CTestView::DoubleBuffer(CDC* pDC)//双缓冲
{
	...
	memDC.FillSolidRect(rect, RGB(0,128,0));//按原来背景填充客户区，否则是黑色
