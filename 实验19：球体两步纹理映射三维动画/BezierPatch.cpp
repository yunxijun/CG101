...
void CBezierPatch::Tessellation(CMesh Mesh)//细分曲面函数
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		quadrT[i] = TwoPartTextureMapping(quadrP[i]);
	}
}
...
CT2 CBezierPatch::TwoPartTextureMapping(CP3 WorldPoint)//两步纹理映射
{
	double H = 600;//圆柱高
	double Theta0 = PI/6, h0 = 350;//标定纹理在圆柱上的位置
	double c = 3, d = 1.0;//纹理宽度和高度方向缩放参数
	double Theta = atan2(WorldPoint.x ,WorldPoint.z);// "mapping to intermediate Surface" [-Pi,Pi]
	double h = WorldPoint.y;
	CT2 Texture;//纹理
	Texture.u = (c * (Theta + Theta0)) / (2 * PI) * (pTexture->bmp.bmWidth - 1);//"mapping to Object"
	Texture.v = (1 / d * (h + h0) / H) * (pTexture->bmp.bmHeight - 1);
	return Texture;	
}