...
CTestView::CTestView() noexcept
{
    ...
    revo[0].ReadCubicBezierControlPoint(V0);	
	tran[0].SetMatrix(revo[0].GetVertexArrayName(), 48);
	revo[1].ReadCubicBezierControlPoint(V1);	
	tran[1].SetMatrix(revo[1].GetVertexArrayName(), 48);
	revo[2].ReadCubicBezierControlPoint(V2);	
	tran[2].SetMatrix(revo[2].GetVertexArrayName(), 48);
	revo[3].ReadCubicBezierControlPoint(V3);	
	tran[3].SetMatrix(revo[3].GetVertexArrayName(), 48);
	InitializeLightingScene();
	texture[0].PrepareBitmap(IDB_BITMAP1);//左面
	texture[1].PrepareBitmap(IDB_BITMAP2);//右面
	texture[2].PrepareBitmap(IDB_BITMAP3);//顶面
	texture[3].PrepareBitmap(IDB_BITMAP4);//底面
	texture[4].PrepareBitmap(IDB_BITMAP5);//前面
	texture[5].PrepareBitmap(IDB_BITMAP6);//后面
	for (int i = 0; i < 4; i++)
	{
		revo[i].patch.SetScene(pLight, pMaterial);//设置场景
		revo[i].patch.SetTexture(texture);//设置纹理
	}
}
...
void CTestView::DrawObject(CDC* pDC)//绘制图形
{
	...
	pZBuffer->InitialDepthBuffer(2000, 2000, 2000);//初始化深度缓冲器
	for (int i = 0; i < 4; i++)
		revo[i].DrawRevolutionSurface(pDC, pZBuffer);
	delete pZBuffer;
}
