...
void CBezierPatch::DrawFacet(CDC* pDC)
{
	...
	for (int nPoint = 0; nPoint < 4; nPoint++)
		ScreenPoint[nPoint] = ObliqueProjection(quadrP[nPoint]);//斜投影
...
void CBezierPatch::DrawControlGrid(CDC* pDC)//绘制控制网格
{
	...
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 4; j++)
			P2[i][j] = ObliqueProjection(CtrPt[i][j]);//斜投影
	CPen NewPen, *pOldPen;
...
CP2 CBezierPatch::ObliqueProjection(CP3 WorldPoint)//斜二测投影
{
...
void CBezierPatch::MarkControlGrid(CDC *pDC)//标注控制点的编号
{
	CString str;
	...
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 4; j++)
		{
			...
			pDC->TextOutW(ROUND(P2[i][j].x - 40), ROUND(P2[i][j].y + 20), str);
		}
}