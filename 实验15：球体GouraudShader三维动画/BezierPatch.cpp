...
void CBezierPatch::DrawFacet(CDC* pDC)
{
	...
	ViewVector = ViewVector.Normalize();//视向量归一化
	...
	if(DotProduct(ViewVector, FacetNormal) >= 0)//背面剔除算法
	{
		...
		{	
			ScreenPoint[nPoint] = projection.PerspectiveProjection2(quadrP[nPoint]);//透视投影
			ScreenPoint[nPoint].c = pLight->Illuminate(ViewPoint, quadrP[nPoint], CVector3(quadrP[nPoint]), pMaterial);//调用光照函数
		}
